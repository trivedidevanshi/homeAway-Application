...
import { getPropertyDashboardQuery } from '../../queries/queries'
...
class ownerPropertyDashboard extends Component {
    ...
    componentDidMount() {
        ...
        this.props.client.query({
            query: getPropertyDashboardQuery,
            variables: {
                email: tokenemail
            }
        }).then(res => {
            console.log("The response is : ", res.data.getPropertyDashboardQuery.properties);
            console.log("The length is : ", res.data.getPropertyDashboardQuery.properties[0].propname);

            let i = 0;
                    for (i = 0; i < res.data.getPropertyDashboardQuery.properties.length; i++) {
                        this.setState({
                            rdata: this.state.rdata.concat([{
                                propName: res.data.getPropertyDashboardQuery.properties[i].propname,
                                propDescription: res.data.getPropertyDashboardQuery.properties[i].propdescription,
                                address: res.data.getPropertyDashboardQuery.properties[i].address,
                                city: res.data.getPropertyDashboardQuery.properties[i].city,
                                state: res.data.getPropertyDashboardQuery.properties[i].state,
                                country: res.data.getPropertyDashboardQuery.properties[i].country,
                                zipcode: res.data.getPropertyDashboardQuery.properties[i].zipcode,
                                propPhotos: res.data.getPropertyDashboardQuery.properties[i].propPhotos,
                                bookeddates: res.data.getPropertyDashboardQuery.properties[i].bookeddates
                            }])
                        });
                    }
                    this.setState({
            ...
            DisplayData = (
                ...
                Object.keys(this.state.rdata).map(function (key) {
                    ...
                    if (key != 0) {
                        ...
                        /////////////////////////////////////////////////////////////
                        console.log("___________booked dates____________", propName);
...
export default withApollo(ownerPropertyDashboard);